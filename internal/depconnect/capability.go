// Copyright 2026 The OpenChoreo Authors
// SPDX-License-Identifier: Apache-2.0

package depconnect

import (
	"crypto/ed25519"
	"fmt"

	"github.com/golang-jwt/jwt/v5"
)

// capabilitySigningMethod is EdDSA (Ed25519). Ed25519 keys are small and fast, and
// golang-jwt supports them out of the box.
var capabilitySigningMethod = jwt.SigningMethodEdDSA

// CapabilityAudience is the JWT audience for dep-connect capabilities. Under the
// current architecture (worklog §8) the capability is minted and verified by the
// same openchoreo-api process — there is no separate agent to distribute a
// verification key to — so the audience is a fixed constant rather than scoped per
// data plane; it exists only as a defense-in-depth check against token reuse across
// unrelated JWT purposes.
const CapabilityAudience = "openchoreo-api:dep-connect"

// ComponentRef identifies the consuming component a capability is scoped to.
type ComponentRef struct {
	Project string `json:"project"`
	Name    string `json:"name"`
}

// Target is a single dialable destination the capability authorizes. The control
// plane resolves the concrete host:port and the owning data plane at resolve time
// and signs them here, so the dep-connect stream endpoint dials only CP-authorized
// targets and never a free-form destination from the client (worklog D9). Key is the
// stable identifier the client references when opening a stream.
type Target struct {
	Key   string `json:"key"`
	Proto string `json:"proto"` // "tcp" for v1
	Host  string `json:"host"`
	Port  int    `json:"port"`

	// PlaneType/PlaneID/CRNamespace/CRName identify the data plane whose cluster-agent
	// connection the cluster-gateway should relay this stream through (mirrors the
	// exec/wirelogs routing convention in internal/cluster-gateway).
	PlaneType   string `json:"planeType"`
	PlaneID     string `json:"planeID"`
	CRNamespace string `json:"crNamespace"`
	CRName      string `json:"crName"`
}

// CapabilityClaims are the custom claims of the dep-connect capability JWT. The
// registered claims carry iss/sub/aud/exp/iat/jti.
type CapabilityClaims struct {
	jwt.RegisteredClaims
	Namespace string       `json:"namespace"`
	Component ComponentRef `json:"component"`
	Env       string       `json:"env"`
	Targets   []Target     `json:"targets"`
}

// TargetByKey returns the authorized target with the given key, if present.
func (c *CapabilityClaims) TargetByKey(key string) (Target, bool) {
	for _, t := range c.Targets {
		if t.Key == key {
			return t, true
		}
	}
	return Target{}, false
}

// SignCapability mints a compact capability JWT signed with the control plane's
// Ed25519 private key. kid is set in the JWT header so the verifier can select the
// matching public key during rotation.
func SignCapability(claims *CapabilityClaims, priv ed25519.PrivateKey, kid string) (string, error) {
	tok := jwt.NewWithClaims(capabilitySigningMethod, claims)
	if kid != "" {
		tok.Header["kid"] = kid
	}
	return tok.SignedString(priv)
}

// VerifyCapability parses and validates a capability JWT against the control plane's
// Ed25519 public key, enforcing the signing method, a present-and-unexpired exp, and
// the dep-connect audience. On success it returns the claims.
func VerifyCapability(token string, pub ed25519.PublicKey) (*CapabilityClaims, error) {
	claims := &CapabilityClaims{}
	parser := jwt.NewParser(
		jwt.WithValidMethods([]string{capabilitySigningMethod.Alg()}),
		jwt.WithExpirationRequired(),
		jwt.WithAudience(CapabilityAudience),
	)
	if _, err := parser.ParseWithClaims(token, claims, func(t *jwt.Token) (any, error) {
		return pub, nil
	}); err != nil {
		return nil, fmt.Errorf("depconnect: verify capability: %w", err)
	}
	return claims, nil
}
