...
// SchemaToolsetHandler handles schema/CRD operations
type SchemaToolsetHandler interface {
	ListCRDs(ctx context.Context) (string, error)
	GetCRD(ctx context.Context, crdName string) (string, error)
}

// ResourceToolsetHandler handles resource instance operations
type ResourceToolsetHandler interface {
	ApplyResource(ctx context.Context, yaml string) (string, error)
	GetResource(ctx context.Context, kind, name, namespace string) (string, error)
	DeleteResource(ctx context.Context, kind, name, namespace string) (string, error)
...
func (t *Toolsets) RegisterListCRDs(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Name: "list_crds",
		Description: "List all OpenChoreo Custom Resource Definitions (CRDs). CRDs define the schema for " +
			"OpenChoreo resources like Components, Projects, Environments, etc. For Kubernetes users: Similar " +
			"to 'kubectl get crds'.",
		...
		result, err := t.SchemaToolset.ListCRDs(ctx)
		return handleToolResult(result, err)
	})
}

func (t *Toolsets) RegisterGetCRD(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Meta: mcp.Meta{
			"toolset": "schema-toolset",
		},
		Name: "get_crd",
		Description: "Get the complete OpenAPI schema for an OpenChoreo Custom Resource Definition. Returns " +
			"full schema with all fields, validation rules, and documentation. Use this to understand resource " +
			"structure before using apply_resource.",
		InputSchema: createSchema(map[string]any{
			"crd_name": stringProperty(
				"CRD name with group (e.g., 'components.openchoreo.dev', 'projects.openchoreo.dev')"),
		}, []string{"crd_name"}),
	}, func(ctx context.Context, req *mcp.CallToolRequest, args struct {
		CRDName string `json:"crd_name"`
	}) (*mcp.CallToolResult, map[string]string, error) {
		result, err := t.SchemaToolset.GetCRD(ctx, args.CRDName)
		return handleToolResult(result, err)
	})
}
...
func (t *Toolsets) RegisterApplyResource(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Name: "apply_resource",
		Description: "Create or update an OpenChoreo resource from JSON. If the resource exists, it will be " +
			"updated; otherwise created. JSON must include kind, apiVersion, metadata.name, and spec. For " +
			"Kubernetes users: Similar to 'kubectl apply -f'.",
		InputSchema: createSchema(map[string]any{
			"json": stringProperty("Complete JSON resource definition (kind, apiVersion, metadata, spec)"),
		}, []string{"json"}),
	}, func(ctx context.Context, req *mcp.CallToolRequest, args struct {
		JSON string `json:"json"`
	}) (*mcp.CallToolResult, map[string]string, error) {
		result, err := t.ResourceToolset.ApplyResource(ctx, args.JSON)
		return handleToolResult(result, err)
	})
}
...
func (t *Toolsets) RegisterGetResource(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Name: "get_resource",
		Description: "Get an OpenChoreo resource by kind, name, and namespace. Returns complete resource " +
			"including spec and status. For Kubernetes users: Similar to 'kubectl get <kind> <name> -o json'.",
		InputSchema: createSchema(map[string]any{
			"kind": stringProperty(
				"E.g., 'Component', 'Project', 'Environment'. Use list_crds to discover all kinds"),
			"name": defaultStringProperty(),
			"namespace": stringProperty(
				"Kubernetes namespace. Leave empty for cluster-scoped resources " +
					"(Organization, DataPlane, Environment are cluster-scoped)"),
		}, []string{"kind", "name"}),
	}, func(ctx context.Context, req *mcp.CallToolRequest, args struct {
		...
		result, err := t.ResourceToolset.GetResource(ctx, args.Kind, args.Name, args.Namespace)
		return handleToolResult(result, err)
	})
}
...
func (t *Toolsets) RegisterDeleteResource(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Delete an OpenChoreo resource by kind, name, and namespace. Permanently removes the " +
			"resource and may cascade to owned resources (e.g., deleting a Project deletes all Components). " +
			"For Kubernetes users: Similar to 'kubectl delete'.",
		InputSchema: createSchema(map[string]any{
			"kind":      stringProperty("E.g., 'Component', 'Project', 'Environment'"),
			"name":      defaultStringProperty(),
			"namespace": stringProperty("Kubernetes namespace. Leave empty for cluster-scoped resources"),
		}, []string{"kind", "name"}),
	}, func(ctx context.Context, req *mcp.CallToolRequest, args struct {
		Kind      string `json:"kind"`
		Name      string `json:"name"`
		...
		result, err := t.ResourceToolset.DeleteResource(ctx, args.Kind, args.Name, args.Namespace)
		return handleToolResult(result, err)
	})
}

func (t *Toolsets) RegisterListResources(s *mcp.Server) {
	mcp.AddTool(s, &mcp.Tool{
		Name: "list_resources",
		Description: "List all OpenChoreo resources of a specific kind. For Kubernetes users: Similar to " +
			"'kubectl get <kind> --all-namespaces'.",
		InputSchema: createSchema(map[string]any{
			"kind": stringProperty("E.g., 'Component', 'Project', 'Environment'"),
			"namespace": stringProperty(
				"Kubernetes namespace. Leave empty to query all namespaces (for namespaced kinds) or " +
		...
		result, err := t.ResourceToolset.ListResources(ctx, args.Kind, args.Namespace)
		return handleToolResult(result, err)
	})
}
